    ...
    server: {
      ...
      proxy: {
        '/api/topology': {
          ...
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/topology/, ''),
        },
        '/api/waystation': {
          target: env.VITE_WAYSTATION_URL || 'http://localhost:1338',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/waystation/, ''),
        },
        '/api/scheduler': {
        ...
        '/api/dag-registry': {
          ...
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/dag-registry/, ''),
        },
        '/api/secure-store': {
          target: env.VITE_SECURE_STORE_URL || 'http://localhost:1343',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/secure-store/, ''),
        },
        '/api/billing': {
        ...
        '/api/blob-store': {
          ...
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/blob-store/, ''),
        },
        '/api/origin': {
