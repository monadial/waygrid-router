...
abstract class ValueRefined[V, P](using
  ...
  protected inline final def derive[F[_]](using ev: F[V], evIso: TypeEvidence[V, Type], F: Functor[F]): F[Type] = ev.mapIso

  extension (t: Type) inline def unwrap: V = t

  given TypeEvidence[Refined[V, P], Type] with
    def iso: Iso[Refined[V, P], Type] = Iso[Refined[V, P], Type](identity)(identity)
