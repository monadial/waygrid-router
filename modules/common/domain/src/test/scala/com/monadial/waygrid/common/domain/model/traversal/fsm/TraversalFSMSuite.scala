 ...
 */
object TestIds:
  import com.monadial.waygrid.common.domain.model.traversal.dag.Value.{ BranchId, ForkId }
...
import TestIds.{ branchId as testBranchId, forkId as testForkId }
...
object TraversalFSMSuite extends SimpleIOSuite:
  ...
  private val retryDag = Dag(
    ...
    nodes = Map(
      nodeA -> Node(
        nodeA,
        Some("Retryable"),
        RetryPolicy.Linear(1.second, maxRetries = 2),
  ...
  private val forkJoinOrDag =
    forkJoinDag.copy(
      ...
      nodes = forkJoinDag.nodes.updated(
        joinNode,
        forkJoinDag.nodes(joinNode).copy(nodeType = NodeType.Join(forkId, JoinStrategy.Or, None))
      )
    )
  ...
  private val forkJoinTimeoutDag =
    forkJoinDag.copy(
      ...
      nodes = forkJoinDag.nodes.updated(
        joinNode,
        forkJoinDag.nodes(joinNode).copy(nodeType = NodeType.Join(forkId, JoinStrategy.And, Some(1.second)))
      ),
  ...
  private def runFsm(
    ...
    now: java.time.Instant = testNow
  ): IO[com.monadial.waygrid.common.domain.model.fsm.Value.Result[TraversalState, TraversalError, TraversalEffect]] =
  ...
  test("NodeFailure with retry policy schedules retry"):
      for
        ...
        failed <-
          runFsm(retryDag, started.state, TraversalSignal.NodeFailure(state.traversalId, nodeA, Some("timeout")))
      yield failed.output match
  ...
  test("NodeFailure with failure edge transitions via OnFailure"):
      for
        ...
        failed <-
          runFsm(failureDag, started.state, TraversalSignal.NodeFailure(state.traversalId, nodeA, Some("error")))
      yield failed.output match
        case Right(TraversalEffect.DispatchNode(_, nid, _, _)) =>
  ...
  test("NodeFailure without retry or failure edge fails traversal"):
      for
        ...
        started <- runFsm(singleNodeDag, state, TraversalSignal.Begin(state.traversalId))
        failed <-
          runFsm(singleNodeDag, started.state, TraversalSignal.NodeFailure(state.traversalId, nodeA, Some("fatal")))
      yield failed.output match
  ...
  test("Vector clock advances on each state transition"):
      ...
      yield expect(v1 > v0) and expect(v2 > v1)
  ...
  test("NodeStart is idempotent"):
      ...
      yield (ack1.output, ack2.output) match
  ...
  test("Fork/Join OR completes on first branch and cancels the other branch"):
      ...
      yield
        ...
        dispatchAfterJoin and expect(!r3.state.hasActiveForks) and expect(r3.state.branchStates.isEmpty) and expect(
          canceledRecorded
        )
  ...
  private val nestedForkDag = Dag(
    ...
    nodes = Map(
      ...
      joinNode2 -> Node(
        ...
        DeliveryStrategy.Immediate,
        svcA,
        NodeType.Join(forkId2, JoinStrategy.And, None)
      )
    ),
    edges = List(
      ...
      val invalidDag = Dag(
        ...
        nodes = Map(
          ...
          join1 -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkId3, JoinStrategy.And, None)
          ),
          join2 -> Node(
            join2,
            Some("Join2"),
  ...
  test("Branch failure in AND join: fails the entire join"):
      for
        ...
        r3 <-
          runFsm(forkJoinDag, r2.state, TraversalSignal.NodeFailure(state0.traversalId, nodeB, Some("branch failure")))
      yield
  ...
  test("Retry inside fork branch: branch retries and continues"):
      ...
      val retryBranchDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeR -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdRetry, JoinStrategy.And, None)
          )
        ),
        edges = List(
      ...
      for
        ...
        r4 <- runFsm(
          retryBranchDag,
          r3.state,
          TraversalSignal.NodeFailure(state0.traversalId, nodeB, Some("temporary error"))
        )
        // Retry B
      ...
      val nestedTimeoutDag = Dag(
        ...
        nodes = Map(
          ...
          join2 -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkId4, JoinStrategy.And, Some(1.second))
          ) // Inner timeout
        ),
        edges = List(
  ...
  test("Out-of-order signals: duplicate BranchComplete is idempotent"):
      ...
      for
        ...
        r4 <- runFsm(
          forkJoinDag,
          r3.state,
          TraversalSignal.BranchComplete(state0.traversalId, branchBId, forkId, BranchResult.Success(None))
        )
      yield
      ...
      val quorumDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeQ -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdQ, JoinStrategy.Quorum(2), None)
          ),
  ...
  test("Failure edge with further traversal continues on failure path"):
      ...
      yield (r2.output, r4.output) match
  ...
  test("CancelBranches signal cancels active branches"):
      for
        ...
        r3 <- runFsm(
          forkJoinDag,
          r2.state,
          TraversalSignal.CancelBranches(
            state0.traversalId,
            forkId,
            Set(branchBId),
            "explicit cancel"
      ...
      val branchTimeoutDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeBT -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdBT, JoinStrategy.And, None)
          ),
      ...
      val scheduledBranchDag = Dag(
        ...
        nodes = Map(
          ...
          nodeXSched -> Node(
            ...
            Some("X-Scheduled"),
            RetryPolicy.None,
            DeliveryStrategy.ScheduleAfter(5.seconds),
            svcB
          ),
          // Y is immediate
          ...
          joinNodeSched -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdSched, JoinStrategy.And, None)
          ),
      ...
      for
        ...
        r2 <- runFsm(
          ...
          TraversalSignal.NodeSuccess(
            state0.traversalId,
            nodeA,
            Some(Json.obj("path" -> Json.fromString("B")))
          )
        )
      ...
      for
        ...
        r2 <- runFsm(
          ...
          TraversalSignal.NodeSuccess(
            state0.traversalId,
            nodeA,
            None // Output is irrelevant for condition evaluation
          )
        )
      ...
      val quorum5Dag = Dag(
        ...
        nodes = Map(
          ...
          joinNode5 -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkId5, JoinStrategy.Quorum(3), None)
          ),
  ...
  test("Quorum(4) with 5 branches: 2 fail means quorum impossible - should fail"):
      ...
      val quorum5FailDag = Dag(
        ...
        nodes = Map(
          ...
          joinNode5F -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkId5F, JoinStrategy.Quorum(4), None)
          ),
          nodeD -> Node(nodeD, Some("After"), RetryPolicy.None, DeliveryStrategy.Immediate, svcA)
        ),
        edges = List(
      ...
      for
        ...
        // N1F fails
        r3 <-
          runFsm(quorum5FailDag, r2.state, TraversalSignal.NodeFailure(state0.traversalId, node1F, Some("N1F failed")))
        // N2F fails - now only 3 can succeed, but Quorum(4) needs 4
        r4 <-
          runFsm(quorum5FailDag, r3.state, TraversalSignal.NodeFailure(state0.traversalId, node2F, Some("N2F failed")))
      yield
      ...
      val nestedOrDag = Dag(
        ...
        nodes = Map(
          ...
          innerJoin -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(innerForkId, JoinStrategy.Or, None)
          ),
          outerJoin -> Node(
            outerJoin,
            Some("OuterJoin"),
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(outerForkId, JoinStrategy.Or, None)
          ),
      ...
      val mixedDag = Dag(
        ...
        nodes = Map(
          ...
          innerOrJoin -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcB,
            NodeType.Join(innerOrForkId, JoinStrategy.Or, None)
          ),
          outerAndJoin -> Node(
            outerAndJoin,
            Some("AndJoin"),
            ...
            DeliveryStrategy.Immediate,
            svcC,
            NodeType.Join(outerAndForkId, JoinStrategy.And, None)
          ),
      ...
      val forkWithTimeoutDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeTO -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdTO, JoinStrategy.And, None)
          ),
          nodeD -> Node(nodeD, Some("After"), RetryPolicy.None, DeliveryStrategy.Immediate, svcD)
        ),
        edges = List(
  ...
  test("Conditional edge on failure path routes correctly"):
      ...
      for
        ...
        r2 <- runFsm(
          failCondDag,
          r1.state,
          TraversalSignal.NodeFailure(state0.traversalId, nodeA, Some("expected failure"))
        )
        ...
        r3 <- runFsm(
          ...
          TraversalSignal.NodeSuccess(
            state0.traversalId,
            nodeB,
            Some(Json.obj("recover" -> Json.fromString("fast")))
          )
        )
      yield r3.output match
        case Right(TraversalEffect.DispatchNode(_, nid, _, _)) =>
  ...
  test("Retry exhaustion inside fork branch fails the branch"):
      ...
      val retryExhaustDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeRE -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdRE, JoinStrategy.And, None)
          ),
          nodeD -> Node(nodeD, Some("After"), RetryPolicy.None, DeliveryStrategy.Immediate, svcD)
        ),
        edges = List(
      ...
      for
        ...
        r3 <-
          runFsm(retryExhaustDag, r2.state, TraversalSignal.NodeFailure(state0.traversalId, nodeXRE, Some("fail 1")))
        // Retry signal
        ...
        r5 <-
          runFsm(retryExhaustDag, r4.state, TraversalSignal.NodeFailure(state0.traversalId, nodeXRE, Some("fail 2")))
        // Retry signal
        r6 <- runFsm(retryExhaustDag, r5.state, TraversalSignal.Retry(state0.traversalId, nodeXRE))
        // X fails third time - retries exhausted
        r7 <-
          runFsm(retryExhaustDag, r6.state, TraversalSignal.NodeFailure(state0.traversalId, nodeXRE, Some("fail 3")))
      yield
      ...
      val onTimeoutDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeOT -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcA,
            NodeType.Join(forkIdOT, JoinStrategy.And, Some(5.seconds))
          ),
          timeoutHandler -> Node(
            timeoutHandler,
            Some("TimeoutHandler"),
      ...
      val deepDag = Dag(
        ...
        nodes = Map(
          ...
          joinNode3 -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcB,
            NodeType.Join(forkL3, JoinStrategy.And, None)
          ),
          joinNode2 -> Node(
            joinNode2,
            Some("Join2"),
            ...
            DeliveryStrategy.Immediate,
            svcC,
            NodeType.Join(forkL2, JoinStrategy.And, None)
          ),
          joinNode1 -> Node(
            joinNode1,
            Some("Join1"),
            ...
            DeliveryStrategy.Immediate,
            svcD,
            NodeType.Join(forkL1, JoinStrategy.And, None)
          ),
      ...
      val emptyBranchDag = Dag(
        ...
        nodes = Map(
          ...
          joinNodeEmpty -> Node(
            ...
            DeliveryStrategy.Immediate,
            svcC,
            NodeType.Join(forkIdEmpty, JoinStrategy.And, None)
          ),
          nodeD -> Node(nodeD, Some("After"), RetryPolicy.None, DeliveryStrategy.Immediate, svcD)
        ),
        edges = List(
