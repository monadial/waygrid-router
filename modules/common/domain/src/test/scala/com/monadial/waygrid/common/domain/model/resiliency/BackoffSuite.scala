...
object BackoffSuite extends SimpleIOSuite:
  ...
  private def roughlyWithin(d: FiniteDuration, max: FiniteDuration, tolerance: Double = 0.08): Boolean =
  ...
  test("Linear backoff with hashKey is deterministic and distinct per key"):
      ...
      val k1 = Some(HashKey.fromString("waystation-A"))
      val k2 = Some(HashKey.fromString("waystation-B"))
      for
  ...
  test("BoundedExponential never exceeds cap (with jitter tolerance)"):
      ...
      for
        delays <- (1 to 10).toList.traverse(n =>
          Backoff[RetryPolicy].nextDelay(policy, n, Some(HashKey.fromString("node-x"))).pure
        )
  ...
  test("Hash jitter symmetric stays within ±maxPercent"):
      ...
      for
        d <- Backoff[RetryPolicy].nextDelay(p, 1, Some(HashKey.fromString("hash-symmetric"))).pure
      yield
  ...
  test("Hash jitter positive only tends to increase delay (≈ within [base, base+max%])"):
      val p = RetryPolicy.Linear(1.second, 3)
      for
        d <- Backoff[RetryPolicy].nextDelay(p, 1, Some(HashKey.fromString("hash-positive"))).pure
      yield
  ...
  test("Hash jitter negative only tends to decrease delay (≈ within [base-max%, base])"):
      val p = RetryPolicy.Linear(1.second, 3)
      for
        d <- Backoff[RetryPolicy].nextDelay(p, 1, Some(HashKey.fromString("hash-negative"))).pure
      yield
  ...
  test("Hash jitter produces stable delay for same key across runs"):
      ...
      val key = Some(HashKey.fromString("stable-hash-key"))
      for
