  ...
  test("DagCompiler compiles multi-entry fork/join spec with matching forkId"):
      ...
      DagCompiler.default[IO].use { compiler =>
        compiler.compile(spec, RouteSalt("test-salt")).map { compiled =>
          ...
          val entryNodes =
            dag.nodes.values.filter(n => n.address == svcEntry1 || n.address == svcEntry2).map(_.id).toList
  ...
  test("DagCompiler is deterministic for fork branch map ordering (same salt)"):
      ...
      DagCompiler.default[IO].use { compiler =>
        for
          ...
          dag1 = compiled1.dag
          dag2 = compiled2.dag
        yield expect.same(dag1.hash, dag2.hash) && expect.same(dag1.nodes.keySet, dag2.nodes.keySet) && expect.same(
          dag1.edges,
          dag2.edges
        )
      }
  ...
  test("DagCompiler handles Fork with Quorum join strategy"):
      ...
      DagCompiler.default[IO].use { compiler =>
        compiler.compile(spec, RouteSalt("quorum-join-salt")).map { compiled =>
          val dag = compiled.dag
          val joinNode = dag.nodes.values.find(_.nodeType match
            case NodeType.Join(_, com.monadial.waygrid.common.domain.model.traversal.dag.JoinStrategy.Quorum(2), _) =>
              true
            case _ => false)
