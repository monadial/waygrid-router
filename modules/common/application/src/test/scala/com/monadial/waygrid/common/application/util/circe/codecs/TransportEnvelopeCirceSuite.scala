 ...
 */
object TransportEnvelopeCirceSuite extends SimpleIOSuite with Checkers:
  ...
  private def roundtrip[A: Encoder: Decoder](value: A): Boolean =
    val json    = value.asJson
    val decoded = json.as[A]
    decoded == Right(value)

  private def roundtripDebug[A: Encoder: Decoder](value: A): Either[String, A] =
    val json = value.asJson
    json.as[A].left.map(err => s"Decode error: ${err.getMessage}\nJSON: ${json.spaces2}")
