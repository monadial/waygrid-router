...
import com.monadial.waygrid.common.application.algebra.FSMDecorator
import com.monadial.waygrid.common.domain.algebra.fsm.FSM

object FSMDecoratorSyntax:

  extension [F[+_], S, I, E, O](fsm: FSM[F, S, I, E, O])
    def withDecorator(decorator: FSMDecorator[F, S, I, E, O]): FSM[F, S, I, E, O] =
      (s, i) => decorator.around(s, i)(fsm.transition(s, i))

//
//import com.monadial.waygrid.common.application.algebra.{ FSM as FSMAlg, FSMTransformer as FSMDec }
//
//object FSMSyntax:
//
...
//      FSMDom: (state, input) =>
//        def loop(curr: A, state: S): F[Result[S, B]] =
//          f(curr)
