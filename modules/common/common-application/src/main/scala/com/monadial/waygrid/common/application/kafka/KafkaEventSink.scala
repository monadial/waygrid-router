...
object KafkaEventSinkMetrics:
  def create[F[+_]: Meter]: Resource[F, KafkaEventSinkMetrics[F]] =
    for
      ...
      encodeSuccess   <- Resource.eval(Meter[F].counter[Long]("event_sink_encode_success_total").withDescription().create)
      encodeFailure   <- Resource.eval(Meter[F].counter[Long]("event_sink_encode_failure_total").create)
      encodeLatency   <- Resource.eval(Meter[F].histogram[Long]("event_sink_encode_latency_ms").withUnit("ms").create)
      eventDequeued   <- Resource.eval(Meter[F].counter[Long]("event_sink_event_dequeued_total").create)
      batchSize       <- Resource.eval(Meter[F].histogram[Long]("event_sink_batch_size").withUnit("events").create)
      inFlightBatches <- Resource.eval(Meter[F].upDownCounter[Long]("event_sink_inflight_batches").withUnit("1").create)
      batchFailure    <- Resource.eval(Meter[F].counter[Long]("event_sink_batch_failure_total").withUnit("events").create)
      batchLatency    <- Resource.eval(Meter[F].histogram[Long]("event_sink_batch_latency_ms").withUnit("ms").create)
...
object KafkaEventSink:
  ...
  private def spawnProducer[F[+_]: Async](
    ...
    for
      ...
      given ValueSerializer[F, TransportEnvelope] <- Resource.pure(Serializer.lift[F, TransportEnvelope](x =>
        BytesCodec[Json].encodeToScalar(x.asJson).toArrayUnsafe.pure[F]
      ))
        ...
        Stream
          ...
          .evalMap: chunk =>
            for
              ...
              // DEBUG: Log batch formation and submission
              ...
              ackEffect
                .flatMap: _ =>
                  Clock[F].monotonic.flatMap: endTime =>
                      ...
                      // DEBUG: Log successful ACK with latency
