...
type SupervisedActors[F[+_]] = Map[String, SupervisedActorRef[F, SupervisedRequest]]

final case class Supervise[F[+_]](actor: SupervisedActorRef[F, SupervisedRequest], stopPriority: Int = 0) extends SupervisorRequest
final case class SupervisedStopped[F[+_]](actor: SupervisedActorRef[F, SupervisedRequest]) extends SupervisedRequest

object SupervisorActor:
  def behavior[F[+_]: {Async, Parallel, Logger, HasNode}]: Resource[F, SupervisorActor[F]] =
    for
      actorMapRef <- Resource.eval(Ref.of[F, SupervisedActors[F]](Map.empty))
    yield new SupervisorActor[F]:
      override def receive: ReplyingReceive[F, SupervisorRequest | SupervisedRequest, Any] =
        case Supervise(actor, shutdownPriority) =>
          for
            typedActor <- actor.asInstanceOf[SupervisedActorRef[F, SupervisedRequest]].pure
            _ <- Logger[F].info(s"Supervising actor: ${actor}")
            _ <- actorMapRef.update(_ + (actor.path.toString -> actor.asInstanceOf[SupervisedActorRef[F, SupervisedRequest]]))
        ...
        case SupervisedStopped(actor) =>
          for
            ...
            _ <- children.find(x => x.path == typedActor.path) match
              case Some(_) =>
                Logger[F].info(s"Actor ${actor} is still alive, not removing from map")
              case None =>
      ...
      override def postStop: F[Unit] =
        for
          ...
          _ <- supervisedActors.toList.parTraverse_ { case (name, childRef) =>
          ...
          _ <- Logger[F].info("Supervisor: all supervised actors truly stopped.")
