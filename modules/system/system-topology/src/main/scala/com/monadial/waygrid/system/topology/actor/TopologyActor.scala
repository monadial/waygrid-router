...
import com.monadial.waygrid.system.topology.actor.TopologyRequest.{RegisterNode, RegisteredNodes}
...
object TopologyActor:
  def behavior[F[+_]: {Async, HasNode, Parallel, Logger}]: Resource[F, TopologyActor[F]] =
    for
      topology <- Resource.eval(Ref[F].of(Topology.initial))
    yield new TopologyActor[F]:
      override def receive: ReplyingReceive[F, TopologyRequest | SupervisedRequest, Any] =
