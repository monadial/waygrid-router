...
object RedisSchedulerStorage:
  ...
  private given RedisCodec[SchedulerShard, ScheduleToken] = Codecs.derive(
    ...
    SplitEpi[Array[Byte], SchedulerShard](
      bytes => BytesCodec[SchedulerShard].decode(bytes).fold(throw _, identity),
      value => BytesCodec[SchedulerShard].encode(value)
    ),
    SplitEpi[Array[Byte], ScheduleToken](
      bytes => BytesCodec[ScheduleToken].decode(bytes).fold(throw _, identity),
      value => BytesCodec[ScheduleToken].encode(value)
    )
  )
