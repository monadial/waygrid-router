...
object TraversalExecutorActorSuite extends SimpleIOSuite:
  ...
  test("TraversalExecutorActor does not persist state for linear DAG and emits NodeTraversalRequested on Begin"):
      for
        sent <- Ref.of[IO, List[(
          NodeTraversalRequested,
          com.monadial.waygrid.common.domain.model.envelope.EnvelopeStamps
        )]](Nil)
          ...
          given EventSink[IO] =
            new EventSink[IO]:
              def send(
                ...
                event: com.monadial.waygrid.common.domain.model.envelope.DomainEnvelope[
                  ? <: com.monadial.waygrid.common.domain.algebra.messaging.event.Event
                ],
              ...
              def sendBatch(
                events: List[
                  (
                    ...
                    com.monadial.waygrid.common.domain.model.envelope.DomainEnvelope[
                      ? <: com.monadial.waygrid.common.domain.algebra.messaging.event.Event
                    ],
                    Option[SpanContext]
                  )
  ...
  test("TraversalExecutorActor restores non-linear state from repository across events"):
      for
        ...
        saved <- repo.load(traversalId)
        msgs  <- sent.get
      yield expect(saved.exists(_.stateVersion.unwrap >= 2)) &&
